...
var MODES = {
  OWNER_FULL: parseInt('0700', 8),
  OWNER_RW: parseInt('0600', 8)
}
...
function mkdir(affixes, callback) {
  ...
  fs.mkdir(dirPath, MODES.OWNER_FULL, function(err) {
    if (!err) {
...
function mkdirSync(affixes) {
  var dirPath = generateName(affixes, 'd-');
  fs.mkdirSync(dirPath, MODES.OWNER_FULL);
  deleteDirOnExit(dirPath);
  return dirPath;
}
...
function open(affixes, callback) {
  ...
  fs.open(filePath, RDWR_EXCL, MODES.OWNER_RW, function(err, fd) {
    if (!err) {
...
function openSync(affixes) {
  var filePath = generateName(affixes, 'f-');
  var fd = fs.openSync(filePath, RDWR_EXCL, MODES.OWNER_RW);
  deleteFileOnExit(filePath);
...
function createWriteStream(affixes) {
  ...
  var stream = fs.createWriteStream(filePath, {flags: RDWR_EXCL, mode: MODES.OWNER_RW});
  deleteFileOnExit(filePath);
  return stream;
}
